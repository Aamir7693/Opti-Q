...
from .config import DEFAULT_QOA
...
def dominates_strict(metrics1, metrics2, epsilon=1e-6):
    """
    Check standard Pareto dominance (exact).
    ...
    from main import LLMMetrics

    # Handle both tuple (cost, latency, energy, qoa) and LLMMetrics objects
    if isinstance(metrics1, LLMMetrics):
        c1 = metrics1.final_cost
        l1 = metrics1.final_latency
        e1 = metrics1.final_energy
        q1 = metrics1.quality_of_answer
    else:
        c1, l1, e1, q1 = metrics1

    if isinstance(metrics2, LLMMetrics):
        c2 = metrics2.final_cost
        l2 = metrics2.final_latency
        e2 = metrics2.final_energy
        q2 = metrics2.quality_of_answer
    else:
...
def evaluate_plan_cached(plan, df_history, query_type):
    ...
    from main import evaluate_individual_V2
...
def generate_all_mutations(p, max_nodes=5):
    ...
    # Import needed functions from dag_utils
    from dag_utils import (
...
def ParetoClimb(p, df_history, query_type, max_iterations=50, max_nodes=5, alpha=1.0):
    """
    Hill climbing that respects Pareto dominance.

    Matches the Java reference implementation: try all 1-step mutations,
    pick first dominating one, repeat until no improvement found.
    ...
    Args:
        ...
        max_nodes: Maximum nodes allowed (default: 5)
        alpha: Approximation factor for pruning (default: 1.0, unused for now)
    ...
    while improving and iterations < max_iterations:
        ...
        # Try each mutation to find a dominating one
        for mutated_plan in mutations:
            mutated_metrics = evaluate_plan_cached(mutated_plan, df_history, query_type)

            # Check if mutation dominates current plan
            if dominates_strict(mutated_metrics, p_metrics):
...
def RandomPlan(max_nodes):
    ...
    if max_nodes <= 6:
        ...
        from main import random_valid_structure
...
def RandomMOQO(query_type, df_history, timeout_seconds=60, max_nodes=5,
    ...
    while (time.time() - start_time) < timeout_seconds and (max_iterations is None or i <= max_iterations):
        ...
        # Calculate elapsed time for adaptive precision schedule
        elapsed = time.time() - start_time

        # Calculate current alpha using paper's iteration-based schedule
        if use_adaptive_precision:
            # From RMQ (Trummer & Koch, SIGMOD 2016):
            # α = 25 * 0.99^(⌊i/25⌋)
            # Starts at 25 (aggressive early pruning, small cache)
            # Decays by 1% every 25 iterations, approaching 1.0 (exact)
            current_alpha = max(1.0, 25.0 * (0.99 ** (i // 25)))
        else:
            # Use constant alpha for experiments
            current_alpha = alpha_start

        # Approximate Pareto frontier with adaptive precision
        if use_subproblem_caching:
            # Use Algorithm 3: DP-like subproblem caching (V3 corrected)
            ...
            P = ApproximateFrontiers(
                optPlan, P, i, query_type, df_history,
        ...
        if i % 50 == 0:
            ...
            # Display progress
            if use_adaptive_precision:
                print(f"  Iter {i}: {solution_count} solutions, "
                      f"{elapsed:.1f}s elapsed, α={current_alpha:.3f}")
            else:
